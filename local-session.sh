...
sessionId=`curl -X POST -H "Content-Type: application/json" -H "x-api-key: $LOOPWORMS_API_KEY" -d "{\"walletId\": \"$walletId\", \"nftId\" : \"$nftId\", \"title\": \"Title 🏛️\", \"xp\":$xp, \"mapId\": \"$mapId\", \"checkpointId\": \"$checkpointId\", \"f2p\": false}" http://127.0.0.1:8000/session | jq -r '.sessionId'`
