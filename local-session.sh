...
sessionId=`curl -X POST -H "Content-Type: application/json" -H "x-api-key: $LOOPWORMS_API_KEY" -d "{\"walletId\": \"$walletId\", \"nftId\" : \"$nftId\", \"title\": \"Title 🏛️\", \"xp\":$xp, \"mapId\": \"$mapId\", \"checkpointId\": \"$checkpointId\", \"f2p\": false, \"trait\": \"rogue\"}" http://127.0.0.1:8000/session | jq -r '.sessionId'`
IP=${LOCAL_IP:-127.0.0.1}
url=http://$IP:8081/?sessionId=$sessionId
