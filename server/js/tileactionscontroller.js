...
class TileActionsController {
    ...
    async getStageForPlot(nftId, map, tileAction, plot, farmDefinition, world) {
        ...
        if (plot.state === "growing") {
            ...
            return {
                key: "wait",
                name: "Growing " + crop.name,
                waiting: true,
                message: crop.name + " will be ready in " + this.formatRemaining(plot.readyAt),
            };
        }
    ...
    async plantCrop(nftId, map, tileAction, cropKey, farmDefinition, world) {
        ...
        if (this.isLevelLocked(playerLevel, crop.level)) {
            return this.fail(world, nftId, "You need level " + crop.level + " to plant " + crop.name + ".");
        }
        ...
        return this.success(world, nftId, "You planted " + crop.name + ".");
    }
    ...
    async waterCrop(nftId, map, tileAction, plot, farmDefinition, world) {
        ...
        return this.success(world, nftId, crop.name + " is watered.");
    }
    ...
    async harvestCrop(nftId, map, tileAction, plot, farmDefinition, world) {
        ...
        if (!this.isReady(plot)) {
            return this.fail(world, nftId, crop.name + " will be ready in " + this.formatRemaining(plot.readyAt) + ".");
        }
        ...
        if (seedItem && this.random() < crop.seedReturnChance) {
            transactions.push({ nftId, itemId: seedItem, quantity: 1 });
        }

        const rareChance = (crop.rareDrop?.chance || 0) + (plot.rareChanceBonus || 0);
        if (crop.rareDrop && this.random() < rareChance) {
            transactions.push({
                nftId,
                itemId: Types.Entities[crop.rareDrop.item],
                quantity: crop.rareDrop.amount || 1,
            });
        }
        ...
        return this.success(world, nftId, "Harvested " + yieldAmount + " " + crop.name + ".");
    }
    ...
    titleForCrop(crop) {
        return crop.name.charAt(0).toUpperCase() + crop.name.slice(1);
    }
    ...
    getPlantTypeMismatchMessage(crop, tileAction, farmDefinition) {
        ...
        if (allowedPlantTypes.length === 1 && allowedPlantTypes[0] === "potted") {
            return crop.name + " needs open farmland.";
        }

        if (this.getCropPlantType(crop) === "potted") {
            return crop.name + " needs a pot.";
        }

        return crop.name + " cannot be planted here.";
    }
