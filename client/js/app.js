...
define(['jquery', 'storage'], function($, Storage) {
    ...
    var App = Class.extend({
        ...
        initEquipmentIcons: function() {
            ...
            if(armor !== 'firefox') {
                if (scale === 2) {
                    $('#armor').css('background-image', 'url("' + armorPath + '")').css('object-fit', 'cover').css('background-position','-3px -311px');
                } else if (scale === 3) {
                    $("#armor").css('background-image', 'url("' + armorPath + '")').css('background-position','-3px -408px');
                }
            }
        },
