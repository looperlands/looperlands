...
class Player {
  ...
  skip(direction) {
    const self = this;
    let index = self.index + (direction === 'prev' ? -1 : 1);
  ...
  skipTo(index) {
    const self = this;
    const currentHowl = self.playlist[self.index].howl;
    if (currentHowl) {
      currentHowl.stop();
    }
  ...
  toggleVolume() {
    var self = this;
    var display = ($('#volume').css('display') === 'block') ? 'none' : 'block'; // Use .css() to get display property
    setTimeout(function () {
      $('#volume').css('display', display);
    }, (display === 'block') ? 0 : 500);
    $('#volume').attr('class', (display === 'block') ? 'fadein' : 'fadeout'); // Use .attr() to set class attribute
  }
...
const player = new Player([
  { title: 'Inspiration in a Box', file: 'Inspiration_in_a_Box', howl: null },
  { title: 'BASSMINT', file: '2', howl: null },
...
var wave = new SiriWave({
  container: waveform,
  width: $('#MAIZfm').width() * 2,
  height: $('#MAIZfm').height() * 0.4,
  cover: true,
  speed: 0.03,
  amplitude: 0.7,
  frequency: 3
...
const bindControls = () => {
  ...
  const handleVolumeDrag = event => {
    if (window.sliderDown) {
      ...
      const startX = $('#MAIZfm').width() * 0.05;
      const layerX = (x - $('#MAIZfm').offset().left) - startX;
      const per = Math.min(1, Math.max(0, layerX / parseFloat($('#barEmpty').width())));
      player.volume(per);
    }
  };
...
const updateWaveHeight = () => {
  const height = $('#MAIZfm').height() * 0.25;
  const width = $('#MAIZfm').width();
  wave.height = $('#MAIZfm').height() / 2;
  ...
  if (sound) {
    ...
    const posLeft = Math.max((width * barWidth + width * 0.05 - 25), 10);
    $('#sliderBtn').css('left', posLeft + 'px');
  }
};
...
const setupDragging = () => {
  ...
  const startDragging = (e) => {
    if ($('#volume').css('display') == 'block') return;
    e.preventDefault();
    offsetX = e.clientX - $('#MAIZfm').offset().left;
    offsetY = e.clientY - $('#MAIZfm').offset().top;
    isDragging = true;
  ...
  const drag = (e) => {
    ...
    $('#MAIZfm').css('left', (x / window.innerWidth) * 100 + '%');
    $('#MAIZfm').css('top', (y / window.innerHeight) * 100 + '%');
  };
  ...
  $('#MAIZfm').on('mousedown', startDragging);
  $('#MAIZfm').on('touchstart', startDragging);
  $(document).on('mouseup', stopDragging);
  $(document).on('touchend', stopDragging);
};

// Bind controls, update wave height, and setup dragging
bindControls();
updateWaveHeight();
setupDragging();

// Listen for window resize to update the wave height
$(window).on('resize', updateWaveHeight);